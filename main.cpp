...
void setupDataAndModels(){
    const char* filename = "mnist_first_image_grayscale.bmp";
    int width = 0;
    int height = 0;
...
)
{
    ...
    if (neuron_id < output_neurons_tile_size) {
        ...
        // Compute dot product between input vector and weights row for this neuron
        for (int i = 0; i < input_tile_size; ++i) {
            // weights_tile is laid out row-major: weights[neuron_id][i] = weights_tile[neuron_id * input_tile_size + i]
            float weight = weights_tile[neuron_id * input_tile_size + i];
            float input_val = input_tile[i];
            sum += weight * input_val;
        }

        // Store the result in output tile
...
void processTiles_weightStatinary(int numNeurons,
    ...
    int currentTileSize = inputTileSize;
        ...
        if(err != CL_SUCCESS){
            ...
            printf("done creating weight buffer\n");
        }
        ...
        if(err != CL_SUCCESS){
            ...
            printf("done creating input buffer\n");
        }
        ...
        if(err != CL_SUCCESS){
            ...
            printf("done creating output buffer\n");
        }
        ...
        clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&weightsTileBuffer);
    ...
    if(err != CL_SUCCESS){
        ...
        printf("wrote weights buffer\n");
    }
    // fill this in
    err = clEnqueueWriteBuffer(queue, inputTileBuffer, CL_TRUE, 0, currentTileSize *sizeof(float),&hidden_layer1_weights[weightsStartIndex], 0, NULL, NULL);
    if (err != CL_SUCCESS){
    } else {
      printf("wrote input buffer\n");
    }

    err = clEnqueueReadBuffer(queue, outputBuffer, CL_TRUE, 0, outputsNeuronTileSize *sizeof(float), &hidden_layer1_out[weightsStartIndex], 0, NULL, NULL);
    if (err != CL_SUCCESS){
    } else {
...
void processTiles_weightStatinary_CPU(
    ...
    for (int tileIndex = 0; tileIndex < numTiles; ++tileIndex) {
        ...
        matrixMul(input_tile, weights_tile, input_tile_size, output_neurons_tile_size, output_tile);
    }
